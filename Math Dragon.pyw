...
def generate_false(num):
    while 1:
        ...
        if nm + nm1 != num and nm - nm1 != num:
            break
        
    return (nm, nm1, random.choice(['-','+']))
...
def start():
    ...
    while running:
        ...
        record_text = f1.render('Рекорд: '+str(config['record']), 1, WHITE)
...
def set_difficulty(c, val):
    lst = {
        'Hard': {'speed': -5, 'mx': 9999},
        'Medium': {'speed': -3, 'mx': 999},
        'Easy': {'speed': -1, 'mx': 9},
        }
...
menu = pygame_menu.Menu(300, 400, 'Math Dragon',
                        theme=pygame_menu.themes.THEME_BLUE)
names = ['Drago', 'Math', 'pro', 'profi']
menu.add_text_input('Name :', default=random.choice(names))
menu.add_selector('Difficulty :', [('Hard', 3), ('Medium', 2), ('Easy', 1)], onchange=set_difficulty)
